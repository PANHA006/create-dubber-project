...
def generate_ass_file(df, output_path, font_name="Arial", font_size=24, alignment=2, bg_box=False):
    ...
    for _, row in df.iterrows():
        ...
        if trans_text:
            start_ts = format_ass_timestamp(float(row["Start"]))
            end_ts = format_ass_timestamp(float(row["End"]))
...
def merge_fragmented_segments(segments, max_gap=1.5):
    """Merge adjacent fragments while preserving their enclosing timing."""
    if not segments:
        return []
    merged = []
    current = None
    ...
    for segment in segments:
        ...
        if not text:
            continue
        if current is None:
            current = {"id": segment.get("id", 0), "start": segment.get("start", 0.0), "end": segment.get("end", 0.0), "text": text}
            continue
        gap = segment.get("start", 0.0) - current["end"]
        if not current["text"].rstrip().endswith(terminal_marks) and gap <= max_gap:
            current["text"] += " " + text
